...
v1.0.3 through v1.0.7 contracts. Semantic checks enforce typed Evidence, identity,
...
SCHEMA_PATHS = {
    ...
    "1.0.7": (
        SCHEMA_DIR / "public-advisor-schema-v1.0.7.json",
        SCHEMA_DIR / "evidence-manifest-schema-v1.0.7.json",
        SCHEMA_DIR / "identity-review-schema-v1.0.7.json",
    ),
}
...
TYPED_CONTRACT_VERSIONS = {"1.0.4", "1.0.5", "1.0.6", "1.0.7"}
V105_PLUS_CONTRACT_VERSIONS = {"1.0.5", "1.0.6", "1.0.7"}
V107_REQUIRED_OWNER_SCOPE = {
    "publication_adoption_decisions",
    "rejected_and_unresolved_publications",
    "orcid_anomalies",
    "featured_publications",
    "focused_final_web_spot_check",
}
...
def _v107_owner_release_review_issues(identity_review: dict[str, Any]) -> list[ValidationIssue]:
    issues: list[ValidationIssue] = []
    review = identity_review.get("owner_release_review")
    if not isinstance(review, dict):
        return [_issue(
            "OWNER_SCOPED_RELEASE_REVIEW_REQUIRED",
            "identity.owner_release_review",
            "v1.0.7 release requires an explicit scoped Owner release review.",
        )]
    if review.get("status") not in {"approved", "approved_with_notes"}:
        issues.append(_issue(
            "OWNER_SCOPED_RELEASE_REVIEW_REQUIRED",
            "identity.owner_release_review.status",
            "Owner scoped release review must be approved or approved_with_notes.",
        ))
    if review.get("reviewer_role") != "owner":
        issues.append(_issue(
            "OWNER_SCOPED_RELEASE_REVIEW_REQUIRED",
            "identity.owner_release_review.reviewer_role",
            "Scoped release review must be performed by the Owner.",
        ))
    if not isinstance(review.get("reviewed_at"), str) or not _format_valid(review["reviewed_at"], "date"):
        issues.append(_issue(
            "OWNER_SCOPED_RELEASE_REVIEW_REQUIRED",
            "identity.owner_release_review.reviewed_at",
            "Scoped Owner release review requires a valid review date.",
        ))
    scope = set(review.get("review_scope", [])) if isinstance(review.get("review_scope"), list) else set()
    if not V107_REQUIRED_OWNER_SCOPE.issubset(scope):
        issues.append(_issue(
            "OWNER_SCOPED_RELEASE_REVIEW_INCOMPLETE",
            "identity.owner_release_review.review_scope",
            "Scoped Owner review does not cover all mandatory release decisions.",
        ))
    if identity_review.get("advisor_id") not in set(review.get("advisor_ids", [])):
        issues.append(_issue(
            "OWNER_SCOPED_RELEASE_REVIEW_ADVISOR_MISMATCH",
            "identity.owner_release_review.advisor_ids",
            "Scoped Owner review must explicitly include this advisor.",
        ))
    if review.get("known_open_risks_acknowledged") is not True:
        issues.append(_issue(
            "OWNER_OPEN_RISK_ACKNOWLEDGEMENT_REQUIRED",
            "identity.owner_release_review.known_open_risks_acknowledged",
            "Owner must acknowledge retained open risks before release eligibility.",
        ))
    if review.get("exhaustive_review") is not False:
        issues.append(_issue(
            "OWNER_REVIEW_SCOPE_OVERCLAIM",
            "identity.owner_release_review.exhaustive_review",
            "This scoped review must not be represented as exhaustive human approval.",
        ))
    return issues


def _v107_orcid_issues(advisor_identity: dict[str, Any]) -> tuple[list[ValidationIssue], list[ValidationIssue]]:
    gate_issues: list[ValidationIssue] = []
    consistency_errors: list[ValidationIssue] = []
    status = advisor_identity.get("orcid_status")
    classification = advisor_identity.get("orcid_release_classification")
    candidate = advisor_identity.get("candidate_orcid")
    conflicts = advisor_identity.get("conflicting_orcid_candidates", [])
    if classification == "verified_identifier":
        if status != "verified" or not isinstance(candidate, str) or not ORCID_PATTERN.fullmatch(candidate) or conflicts:
            consistency_errors.append(_issue("ORCID_RELEASE_CLASSIFICATION_INCONSISTENT", "identity.advisor_identity", "verified_identifier requires one canonical verified ORCID and no conflicts."))
    elif classification == "identifier_not_found":
        if status != "not_found" or candidate is not None or conflicts:
            consistency_errors.append(_issue("ORCID_RELEASE_CLASSIFICATION_INCONSISTENT", "identity.advisor_identity", "identifier_not_found requires no candidate or conflicting ORCID."))
    elif classification == "optional_identifier_unresolved":
        if status != "unresolved" or not isinstance(candidate, str) or not ORCID_PATTERN.fullmatch(candidate) or conflicts:
            consistency_errors.append(_issue("ORCID_RELEASE_CLASSIFICATION_INCONSISTENT", "identity.advisor_identity", "optional_identifier_unresolved requires one plausible unpublished candidate and no conflict set."))
    elif classification == "conflicting_optional_identifiers":
        if status != "unresolved" or candidate is not None or not isinstance(conflicts, list) or len(set(conflicts)) < 2 or any(not isinstance(value, str) or not ORCID_PATTERN.fullmatch(value) for value in conflicts):
            consistency_errors.append(_issue("ORCID_RELEASE_CLASSIFICATION_INCONSISTENT", "identity.advisor_identity", "conflicting_optional_identifiers requires no selected candidate and at least two canonical conflicting values."))
    else:
        gate_issues.append(_issue("ADVISOR_IDENTITY_NOT_RELEASE_READY", "identity.advisor_identity.orcid_release_classification", "A recognized v1.0.7 ORCID release classification is required."))
    return gate_issues, consistency_errors


def _v107_public_orcid_exposure_issues(
    public: dict[str, Any], manifest: dict[str, Any], identity_review: dict[str, Any], contract_version: str
) -> list[ValidationIssue]:
    if contract_version != "1.0.7":
        return []
    advisor_identity = identity_review.get("advisor_identity", {})
    classification = advisor_identity.get("orcid_release_classification")
    if classification not in {"optional_identifier_unresolved", "conflicting_optional_identifiers"}:
        return []
    unresolved_values = [advisor_identity.get("candidate_orcid")]
    unresolved_values.extend(advisor_identity.get("conflicting_orcid_candidates", []))
    public_text = json.dumps({"public": public, "manifest": manifest}, ensure_ascii=False)
    if any(value and value in public_text for value in unresolved_values):
        return [_issue(
            "UNRESOLVED_ORCID_PUBLIC_EXPOSURE",
            "public_or_manifest",
            "Unresolved or conflicting advisor ORCID must not be exposed in public package artifacts.",
        )]
    return []
...
def _advisor_identity_gate_issues(
    ...
    if contract_version == "1.0.7":
        gate_issues.extend(_v107_owner_release_review_issues(identity_review))
    elif identity_review.get("review_status") != "verified":
    ...
    if contract_version != "1.0.7" and (not isinstance(reviewed_at, str) or not _format_valid(reviewed_at, "date")):
        gate_issues.append(_issue(
    ...
    if contract_version != "1.0.7" and reviewer_role not in HUMAN_IDENTITY_REVIEW_ROLES:
        gate_issues.append(_issue(
    ...
    if identity_review.get("schema_version") in TYPED_CONTRACT_VERSIONS:
        ...
        if contract_version != "1.0.7" and advisor_identity.get("human_review_status") != "verified":
            gate_issues.append(_issue(
                "IDENTITY_HUMAN_REVIEW_REQUIRED",
    ...
    if contract_version == "1.0.7":
        orcid_gate_issues, orcid_consistency_errors = _v107_orcid_issues(advisor_identity)
        gate_issues.extend(orcid_gate_issues)
        consistency_errors.extend(orcid_consistency_errors)
        return gate_issues, consistency_errors
...
def validate_package(
    ...
    *,
    ...
    errors.extend(_v107_public_orcid_exposure_issues(public, manifest, identity_review, contract_version))
    ...
    advisor_gate_issues, orcid_consistency_errors = _advisor_identity_gate_issues(identity_review, contract_version)
    ...
    if contract_version in {"1.0.6", "1.0.7"} and not adopted_publications:
    ...
    p0_blockers = identity_review.get("p0_blockers", [])
    active_p0_blockers = p0_blockers
    if contract_version == "1.0.7":
        active_p0_blockers = [
            blocker for blocker in p0_blockers
            if not isinstance(blocker, dict) or blocker.get("code") != "HUMAN_IDENTITY_REVIEW_PENDING"
        ]
        classification = identity_review.get("advisor_identity", {}).get("orcid_release_classification")
        if classification in {"optional_identifier_unresolved", "conflicting_optional_identifiers"}:
            warnings.append(_issue(
                "OPTIONAL_IDENTIFIER_OPEN_RISK",
                "identity.advisor_identity.orcid_status",
                "Advisor-level ORCID remains unresolved and must not be exposed as verified public data.",
                "warning",
            ))
        if identity_review.get("owner_release_review", {}).get("exhaustive_review") is False:
            warnings.append(_issue(
                "OWNER_SCOPED_REVIEW_NOT_EXHAUSTIVE",
                "identity.owner_release_review",
                "Owner release review is scoped and does not claim exhaustive field-by-field approval.",
                "warning",
            ))
    unresolved_identity = (
        bool(advisor_gate_issues)
        or bool(orcid_consistency_errors)
        or bool(active_p0_blockers)
    ...
    if contract_version in {"1.0.6", "1.0.7"}:
    ...
    if unresolved_identity:
        ...
        if requested_status in PUBLIC_STATUSES:
            ...
            if active_p0_blockers or publication_identity_unresolved:
                errors.append(_issue(
                    "ADVISOR_IDENTITY_NOT_RELEASE_READY",
                    "identity",
