...
export function AdvisorDetailPage() {
  ...
  const phase2Review = advisors.length === 8 && advisors.every(
    (item) => item.version === "1.0.6" && item.releaseEligible === false,
  );
  ...
  return (
    <div className="detail-page">
      ...
      {phase2Review && (
        <aside className="phase2-review-note" aria-label="Phase 2 待审核状态">
          <strong>待 Owner 人工审核 · RELEASE ELIGIBLE: FALSE</strong>
          <span>{advisor.dataStatusNote}</span>
          <span>当前未纳入论文候选证据，代表论文及作者身份尚待检索与核验。</span>
        </aside>
      )}
      ...
      <header className="profile-header">
        ...
        <dl className="profile-meta">
          <div>
            <dt>机构 / 学院</dt>
            <dd>{[advisor.institution, advisor.schoolOrDepartment].filter(Boolean).join(" · ")}</dd>
          </div>
          <div>
            <dt>职位 / 导师类型</dt>
            <dd>{[advisor.position, ...(advisor.publicRoles ?? [])].filter(Boolean).join(" · ")}</dd>
          </div>
          <div>
            <dt>官方主页</dt>
            <dd>
              {advisor.officialHomepage ? (
                <a href={advisor.officialHomepage} target="_blank" rel="noopener noreferrer">
                  查看官方来源 ↗
                </a>
              ) : "待核验"}
            </dd>
          ...
          <div>
            ...
            <dd>
              {phase2Review ? (
                "pending_verification · 候选 0 · 已采用 0"
              ) : (
                <ConfidenceTag
                  level={advisor.authorMatchConfidence}
                  label="Confidence"
                  note={
                    advisor.authorConfidenceSource === "legacy_academic_confidence"
