...
本模板描述 v1.0.5 `public-advisor-v1.json` 经确定性 Renderer 输出的公开 Markdown。Markdown 不是权威数据源，不得反向解析生成 JSON。
...
公开事实只能由合同允许的一手官方 Evidence 支持。AI整理可以基于采用的论文或官方主页，但必须保留条件式表达和本科生任务不确定性。Renderer不声明 Schema 或 Validator 已通过；结论以 `validation-report-v1.json` 为准。v1.0.5 的来源闭合和身份链错误不得通过 Validator。
